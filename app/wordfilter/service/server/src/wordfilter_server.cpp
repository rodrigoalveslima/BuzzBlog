...
class TWordfilterServiceHandler :
    public TWordfilterServiceIf {
public:
  TWordfilterServiceHandler(const int n_invalid_words) {
    if (n_invalid_words > 0)
      invalid_words.push_back("corinthians");
...
int main(int argc, char** argv) {
  ...
  options.add_options()
      ...
      ("n_invalid_words", "", cxxopts::value<int>()->default_value("0"));
  ...
  TThreadedServer server(
      std::make_shared<TWordfilterServiceProcessor>(
          std::make_shared<TWordfilterServiceHandler>(n_invalid_words)),
      socket,
