...
class TestService(unittest.TestCase):
  def test_is_valid_word(self):
    with WordfilterClient(IP_ADDRESS, WORDFILTER_PORT) as client:
      self.assertFalse(client.is_valid_word(TRequestMetadata(id=random_id()), "corinthians"))
      self.assertTrue(client.is_valid_word(TRequestMetadata(id=random_id()), "foobar"))
