...
class TestService(unittest.TestCase):
  ...
  def test_add(self):
    with UniquepairClient(IP_ADDRESS, UNIQUEPAIR_PORT) as client:
      ...
      uniquepair = client.add(TRequestMetadata(id=random_id()),
          TEST_DOMAIN, first_elem, second_elem)
  ...
  def test_find(self):
    with UniquepairClient(IP_ADDRESS, UNIQUEPAIR_PORT) as client:
      # Find unique pair and check its id.
      uniquepair = client.find(TRequestMetadata(id=random_id()),
          TEST_DOMAIN, self._uniquepair.first_elem,
          self._uniquepair.second_elem)
      self.assertEqual(self._uniquepair.id, uniquepair.id)
      # Check that a unique pair that does not exist is not found.
      with self.assertRaises(TUniquepairNotFoundException):
        client.find(TRequestMetadata(id=random_id()),
            TEST_DOMAIN, 42, 42)
  ...
  def test_fetch(self):
    with UniquepairClient(IP_ADDRESS, UNIQUEPAIR_PORT) as client:
      ...
      limit = 10
      offset = 0
      uniquepairs = client.fetch(TRequestMetadata(id=random_id()), query,
          limit, offset)
