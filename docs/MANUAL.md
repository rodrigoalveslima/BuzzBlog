# User Manual
This running example shows how to deploy BuzzBlog in your local machine.
...
a Redis data structure server). In the following, we show how to configure,
start, and test these components.

## Configuration
### `conf/backend.yml`
In `conf/backend.yml`, set the hostnames and ports of microservices and
data storage systems. This file is used by the API Gateway and microservices to
discover what server addresses they should connect to. If you are not certain,
...
### `conf/redis.conf`
In `conf/redis.conf`, set the Redis server configuration parameters. If you are
not certain, do not modify this file.

## Deployment
### API Gateway
1. Generate Thrift client and server stubs, and copy client libraries.
```
...
```
3. Start a Docker container based on the newly built image (environment
...
sudo docker run \
    ...
    --env threads=512 \
    --env port=81 \
    --env microservice_connection_pool_size=16 \
    --detach \
    apigateway:latest
```
...
```
4. Generate Thrift client and server stubs, and copy client libraries.
```
sudo ./utils/generate_and_copy_code.sh
```
...
```
6. Start a Docker container based on the newly built image (environment
variables are documented in the Dockerfile).
```
sudo docker run \
    ...
    --env microservice_connection_pool_size=8 \
    --env postgres_connection_pool_size=16 \
    --env postgres_user=postgres \
    --env postgres_password=postgres \
    --volume $(pwd)/conf/backend.yml:/etc/opt/BuzzBlog/backend.yml \
    --detach \
...
```
3. Generate Thrift client and server stubs, and copy client libraries.
```
sudo ./utils/generate_and_copy_code.sh
```
...
```
5. Start a Docker container based on the newly built image (environment
variables are documented in the Dockerfile).
```
sudo docker run \
